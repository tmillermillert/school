...
public class JdbcLab
{
	...
	public static void main(String[] args)
	{
		...
		if (args.length < 5)
		{
			...
			System.out.println("java ser322.JdbcLab <url> <user> <pwd> <driver> export <filename>");
			System.exit(0);
		}
		...
		try {
			...
			// Step 4: Make a query
			if(args[4].compareTo("query1") == 0 && len == 5){
				jdbcLab.query1();
			}
			else if(args[4].compareTo("query2") == 0 && len == 6){

				jdbcLab.query2(args[5]);

			}
			else if(args[4].compareTo("dml1") == 0 && len == 9){
				jdbcLab.dml1(args[5],args[6],args[7],args[8]);
			}
			else if(args[4].compareTo("export") == 0 && len == 6){
