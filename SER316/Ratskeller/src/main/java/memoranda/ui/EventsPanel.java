...
public class EventsPanel extends JPanel {
    ...
    void jbInit() throws Exception {
        ...
        newEventB.setToolTipText(Local.getString("Schedule a Route"));
        ...
        editEventB.setToolTipText(Local.getString("Edit the Route"));
        ...
        removeEventB.setToolTipText(Local.getString("Remove Route"));
    ...
    void editEventB_actionPerformed(ActionEvent e) {
        EventDialog dlg = new EventDialog(App.getFrame(), Local.getString("Route"));
        ...
        dlg.busDestinationField.setText(ev.getText());
        dlg.busNumberField.setText(ev.getText2());
        //add combobox to event display
        dlg.comboBox.setName(ev.getText());
        //TODO
        ...
        String text = dlg.busDestinationField.getText();
        String text2 = dlg.busNumberField.getText();
        String text3 = dlg.comboBox.getSelectedItem().toString();
        if (dlg.noRepeatRB.isSelected())
   	    EventsManager.createEvent(CurrentDate.get(), hh, mm, text,text2, text3);
        else {
	    updateEvents(dlg,hh,mm,text,text2);
	}    
	saveEvents();
    }
    ...
    void newEventB_actionPerformed(ActionEvent e) {
    	...
    	newEventB_actionPerformed(e, null, null,  cdate.getTime(), cdate.getTime());
    }
    
    void newEventB_actionPerformed(ActionEvent e, String busDestinationText, String busDestExistingText,
                                   Date startDate, Date endDate) {
    	EventDialog dlg = new EventDialog(App.getFrame(), Local.getString("New Route"));
    	Dimension frmSize = App.getFrame().getSize();
    	Point loc = App.getFrame().getLocation();

    	if (busDestinationText != null) {
    		dlg.busDestinationField.setText(busDestinationText);
    	} else if (busDestExistingText != null) {
    	    dlg.comboBox.getSelectedItem();
        }

    	/*if (busNumberText != null) {
    		dlg.busNumberField.setText(busNumberText);
    	}*/
    	...
    	if (dlg.noRepeatRB.isSelected())
    		EventsManager.createEvent(eventCalendarDate, hh, mm, text,text2,text3);
    	else {
    		updateEvents(dlg,hh,mm,text,text2);
    	}
    	saveEvents();
    ...
    private void updateEvents(EventDialog dlg, int hh, int mm, String text,String text2) {
	int rtype;
        int period;
        ...
        EventsManager.createRepeatableEvent(rtype, sd, ed, period, hh, mm, text, text2, dlg.workingDaysOnlyCB.isSelected());
    }
